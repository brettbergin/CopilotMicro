...
.PHONY: help doctor doctor-xcode build package smoke-test test-doctor test-packager test-contracts test-bridge test-cli-probe test-core qualify-cli qualify-terminals qualify-ghostty qualify-ghostty-roundtrip qualify-ghostty-association qualify-hardware observe-device-input qualify-device-lighting preview-device-mapping apply-device-mapping preview-device-restore restore-device-mapping lint format check
...
help:
	@printf '%s\n' \
		...
		'qualify-ghostty-association Verify a temporary Ghostty child inherits its surface token' \
...
qualify-ghostty-association:
	@if [ "$(CONSENT)" != "I-authorize-ghostty-environment-test" ]; then printf '%s\n' 'CONSENT must be I-authorize-ghostty-environment-test' >&2; exit 2; fi
	./scripts/swiftpm run --package-path Packages/CopilotMicroKit --scratch-path Packages/CopilotMicroKit/.build CopilotMicroGhosttyProbe --association-environment --consent="$(CONSENT)"
