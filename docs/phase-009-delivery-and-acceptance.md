...
| U-04 | Exact terminal window/tab/pane targeting | Partial | Ghostty 1.3.1 stable window/tab/terminal observation and exact focus are qualified on macOS 26.6.2, including another foreground app and a temporary multi-tab/split window; Ghostty exposes no child PID or TTY, so an arbitrary existing CLI instance cannot yet be associated safely | Add an exact app-created surface/bridge registration token; qualify iTerm2 and Terminal.app separately |
